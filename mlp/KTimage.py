...
# To see these image files with max and min values from the comment displayed,
# use look.tcl as:
#    look.tcl a w 0 1
...
import numpy
import sys
# Python version "2" or "3" -- global variable (string, not int!)
...
def importimage(filename):
    ...
    # possible comment in file: maximum and minimum values to interprete the
    # data values
    if line[0] == "#":
    ...
    # next line (says the maximum value that the data are scaled to, i.e. the
    # brightness value that is to be displayed as white)
    line = f.readline()
    maxchar = int(line)
    ...
    # write the data into a data vector of length height times width (i.e. the
    # image geometry doesn't matter)
    ...
    if format == "P5":
        ...
        for i in range(len(data)):
            values[i] = float(
                ord(data[i])) / float(maxchar) * (highS - lowS) + lowS
    elif format == "P2":
        liste = data.split()
        if len(liste) != height * width:
            print("len(data liste) does not fit height*width!")
        for i in range(len(liste)):
            values[i] = float(int(liste[i])) / \
                float(maxchar) * (highS - lowS) + lowS
    else:
...
def exportinfo(filename, highS, lowS):
    """used by exporttiles()
    inserts into the exported file a comment which looks e.g. like this:
    # highS: 0.099849  lowS: -0.099849
...
# and also the output neuron layer is seen as a 2-dimensional sheet of
# size x*y.
def exporttiles(X, h, w, filename, x=None, y=None):
    ...
    if x is None:
        # blow the vector up to a matrix with one row
        X = numpy.reshape(X, (1, h * w))
        x, y, = 1, 1
        frame = 0
    else:
        # show one pixel row around each neuron's input weight vector
        frame = 1
    ...
    Y = 0.5 * numpy.ones((frame + x * (h + frame), frame + y * (w + frame)))
